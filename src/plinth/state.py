"""State management for Plinth projects."""
...
@dataclass
class ProjectState:
    """Project state stored in .plinth.json."""
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ProjectState":
        ...
        return cls(
            version=data.get("version", plinth_config.VERSION),
...
class StateManager:
    """Manages the .plinth.json state file."""

    def __init__(self, project_path: Path | str):
        self.project_path = Path(project_path).resolve()
        self.state_file = self.project_path / plinth_config.STATE_FILENAME
    ...
    def load(self) -> ProjectState:
        ...
        Raises:
            NotAPlinthProjectError: If state file doesn't exist
        """
        if not self.exists():
            raise NotAPlinthProjectError(str(self.project_path))

        try:
            data = json.loads(self.state_file.read_text())
            return ProjectState.from_dict(data)
        except json.JSONDecodeError as e:
            raise NotAPlinthProjectError(
